...
public class BestFit {
    ...
    public static void bestFit(List<Process> processes, List<Partitions> partitions) {
        if (!isCompaction) {
        ...
        for (int i = 0; i < processes.size(); i++)
        {
            ...
            for (int j = 0; j < tempPartitions.size(); j++)
            {
                ...
                {
                    if (tempPartitions.get(j).size < min) {
                        min = tempPartitions.get(j).size;
                        index = j;
                        check = true;
                    }
                }
        ...
        if(!isCompaction) {
            ...
            if (isCompaction) {
                ...
                bestFit(processes, tempPartitions);
            }
